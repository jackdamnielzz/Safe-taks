...
describe('HazardSelector', () => {
  it('renders and shows hazards list', async () => {
    ...
    // Expect results count text (default translator returns fallback)
    await waitFor(() => {
  ...
  it('filters hazards by search query', async () => {
    ...
    const search = screen.getByPlaceholderText(/Zoeken/);
    ...
    fireEvent.change(search, { target: { value: 'this-does-not-exist-xyz' } });
    await waitFor(() => {
      // No checkboxes since filtered out
      const items = screen.queryAllByRole('checkbox');
      expect(items.length).toBe(0);
      // No results message or zero count shown via translator fallback
      expect(screen.getByText(/gevaren gevonden/i) || screen.getByText(/0/)).toBeTruthy();
    });
  });
  ...
  it('allows selecting and removing hazards', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/Geselecteerde gevaren|Geselecteerde gevaren/i) || screen.getByText(/geselecteerd/i)).toBeTruthy();
    });
