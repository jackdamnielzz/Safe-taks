...
export default function ApprovalDecisionPanel({
      ...
      case "approved":
        return (
          <Badge variant="success">
            {t("approvals.status.approved", { default: "Goedgekeurd" })}
          </Badge>
        );
      case "rejected":
        return (
          <Badge variant="error">{t("approvals.status.rejected", { default: "Afgewezen" })}</Badge>
        );
      case "pending":
        return (
          <Badge variant="warning">
            {t("approvals.status.pending", { default: "In afwachting" })}
          </Badge>
        );
      default:
  ...
  return (
    <div className="space-y-6">
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    {step.step === approval.currentStep && !isComplete && (
                      <Badge variant="info">
                        {t("approvals.steps.current", { default: "Huidige stap" })}
                      </Badge>
                    )}
                  </div>
                  ...
                  {step.dueDate && (
                    <p className="mt-1 text-xs text-gray-500">
                      {t("approvals.steps.dueDate", {
                        default: "Deadline: {date}",
                        date: new Date(step.dueDate).toLocaleDateString("nl-NL"),
                      })}
                    </p>
                  )}
                </div>
            ...
            <div className="space-y-4">
              <div className="rounded-lg bg-gray-50 p-4">
                <p className="font-medium">
                  {action === "approve" &&
                    t("approvals.decision.confirmApprove", {
                      default: "U staat op het punt deze TRA goed te keuren",
                    })}
                  {action === "reject" &&
                    t("approvals.decision.confirmReject", {
                      default: "U staat op het punt deze TRA af te wijzen",
                    })}
                  {action === "request_changes" &&
                    t("approvals.decision.confirmRequestChanges", {
                      default: "U vraagt wijzigingen aan deze TRA",
                    })}
                </p>
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder={t("approvals.decision.commentsPlaceholder", {
                    default: "Voeg uw opmerkingen toe...",
                  })}
              ...
              <div className="flex gap-3">
                <Button
                  ...
                  variant={
                    action === "approve" ? "primary" : action === "reject" ? "danger" : "primary"
                  }
                >
                  {submitting
      ...
      {isComplete && (
        <div
          className={`rounded-lg p-6 ${approval.status === "approved" ? "bg-green-50" : "bg-red-50"}`}
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <p
                className={`font-semibold ${approval.status === "approved" ? "text-green-900" : "text-red-900"}`}
              >
                {approval.status === "approved"
              ...
              <p
                className={`text-sm ${approval.status === "approved" ? "text-green-700" : "text-red-700"}`}
              >
                {approval.status === "approved"
                  ? t("approvals.status.approvedMessage", {
                      default: "Alle goedkeuringsstappen zijn voltooid",
                    })
                  : t("approvals.status.rejectedMessage", {
                      default: "Deze TRA moet worden herzien en opnieuw ingediend",
                    })}
              </p>
            </div>
          </div>
